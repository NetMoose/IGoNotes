...
База метаданных SQLite остаётся в `~/.igonotes/metadata.db`, а первоначальная база заметок остаётся в `~/.igonotes/bases/default`. Семантическое расположение каталога данных приложения `~/.igonotes` не меняется, но домашний каталог определяется через `os.UserHomeDir()`, а не прямое чтение переменной `HOME`. Если системный домашний каталог определить не удалось или он пуст, запуск завершается с понятной ошибкой. Платформенно-зависимым и совместимым с XDG становится только каталог конфигурации.
